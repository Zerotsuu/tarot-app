        ...
        <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
          ...
          <div
            className={`flex justify-between ${
              checkCards ? "pointer-events-none" : ""
            }`}
          >
            {Array.from({ length: 20 }, (_, i) => (
              <div
                key={i}
                className={`filter h-80 rounded-2xl flex bg-black cursor-pointer  hover:mt-10 ${
                  i !== 0 ? "-ml-52 " : ""
                } ${clickedImages.includes(i) ? "mt-10" : ""} ${
                  clickedImages.includes(i) ? "grayscale" : ""
                }`}
              >
                <img
                  src={cardBack}
